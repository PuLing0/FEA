...
class EvaluatorAgent:
    ...
    def run(self, state: RuntimeState) -> RuntimeState:
        ...
        if evaluate_execution.status == "failed":
            decision = self._build_evaluate_failure_replan_decision(
                state=state,
                task_id=current_task_id,
                error=evaluate_execution.error or {},
            )
            task_state.latest_evaluate_checkpoint = "failed"
            self._clear_task_retry_context(task_state)
            task_state.status = TaskStatus.REPLANNED
        ...
        append_evaluation_message(
            state,
            task_id=current_task_id,
            evaluation_ref=evaluate_execution.artifacts[0].id if evaluate_execution.artifacts else None,
            verdict=evaluation_payload.get("verdict"),
            reason=evaluation_payload.get("reason"),
            candidate_ref=evaluation_payload.get("candidate_ref"),
            candidate_refs=list(evaluation_payload.get("candidate_refs") or []),
            checks=list(evaluation_payload.get("checks") or []),
        )
    ...
    def _find_latest_edit_instruction(self, state: RuntimeState, task_id: str) -> str | None:
        ...
        if result is None:
            return None
        instruction = result.args.get("instruction")
